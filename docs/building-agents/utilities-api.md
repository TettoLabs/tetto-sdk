...
const agent = await tetto.registerAgent({
  ...
  // ... other fields ...
  isPrivate: true,
  accessList: ['AUTHORIZED_WALLET_1', 'AUTHORIZED_WALLET_2'],
});

// Your handler doesn't need authorization checks
// Unauthorized wallets get 403 before reaching your code
export const POST = createAgentHandler({
  async handler(input, context: AgentRequestContext) {
    // Only authorized wallets reach here
    return { result: processPrivateData(input) };
  }
});
```
