...
class WebhookVerifier {
     ...
     */
    constructor(secret, maxTimestampAge = 300) {
        if (!secret || secret.trim() === '') {
            throw new Error('TETTO_ENDPOINT_SECRET is required for webhook verification. ' +
     ...
     */
    verify(signatureHeader, requestBody) {
        ...
        const signaturesMatch = crypto_1.default.timingSafeEqual(providedBuffer, expectedBuffer);
        if (!signaturesMatch) {
            return {
                valid: false,
                error: 'Invalid signature. Request not from Tetto platform.',
 ...
 * ```
 */
function verifyWebhookSignature(secret, signatureHeader, requestBody) {
